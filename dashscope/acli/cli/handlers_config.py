...
def _handle_privacy_command(cmd: str, config: Config) -> None:
    ...
    def _print_status():
        status = "🔒 已启用" if config.privacy_mode else "🔓 未启用"
    ...
    if sub == "on":
        ...
        console.print("  [dim]云端能力禁用 · cron 暂停 · 审计脱敏[/dim]")
        if removed or mcp_names:
            console.print(f"  [dim]已撤销 {removed + len(mcp_names)} 个云端工具[/dim]")
        console.print("  [yellow]注意: LLM API 调用不受控[/yellow]")
        return
    ...
    if sub == "off":
        ...
        console.print("[green]✓ 隐私模式已禁用[/green]")
        return
    ...
    console.print("[dim]用法: /privacy [on|off|status][/dim]")
...
def _handle_debug_command(cmd: str, config: Config) -> None:
    ...
    if sub == "off":
        ...
        console.print("[green]✓ 调试模式已关闭[/green]")
        return

    if sub == "status":
        status = "已启用" if config.debug else "未启用"
        console.print(f"[bold]调试模式:[/bold] {status}")
        if config.debug:
            console.print(
    ...
    console.print("[dim]用法: /debug [on|off|status][/dim]")
...
def _handle_log_command(cmd: str, agent=None) -> None:
    ...
    if sub == "clear":
        if tracer is not None:
            tracer.clear()
        console.print("[green]✓ 会话 trace 已清空[/green]")
        return
    ...
    if sub == "search":
        ...
        if not keyword:
            console.print("[yellow]用法: /log search <关键词>[/yellow]")
            return
        kw = keyword.lower()
        events = [
            e
        ...
        header = f'LLM 请求搜索 "{keyword}"（{len(events)} 条）'
    elif sub == "tail":
        limit = 3
        if len(parts) > 2:
            try:
                ...
                console.print("[yellow]用法: /log tail [N]（N 为正整数）[/yellow]")
                return
        events = events[-limit:]
        header = f"LLM 请求最近 {len(events)} 条"
    else:
        console.print("[dim]用法: /log [tail [N] | search <关键词> | clear][/dim]")
        return
...
def _handle_trace_command(cmd: str, agent=None) -> None:
    ...
    if sub == "search":
        keyword = parts[2].strip() if len(parts) > 2 else ""
        if not keyword:
            console.print("[yellow]用法: /trace search <关键词>[/yellow]")
            return
        events = tracer.search_events(keyword) if tracer is not None else []
        header = f'trace 搜索 "{keyword}"（{len(events)} 条）'
    elif sub == "tail":
        limit = 5
        if len(parts) > 2:
            try:
                ...
                console.print("[yellow]用法: /trace tail [N]（N 为正整数）[/yellow]")
                return
    ...
    if not events:
        console.print("[dim]暂无 trace 记录（本轮对话产生调用后自动记录）[/dim]")
        return
...
def _handle_theme_command(cmd: str, config: Config) -> None:
    ...
    if sub == "set":
        ...
        if not theme_name or theme_name not in THEME_PRESETS:
            ...
            console.print(f"[dim]可用: {', '.join(THEME_PRESETS.keys())}[/dim]")
            return
    ...
    if len(parts) >= 3 and sub in config.theme:
        ...
        config.save_workspace()
        console.print(f"[green]✓ 已设置 {sub} = {parts[2]}[/green]")
        console.print("[dim]重启后生效[/dim]")
        return

    console.print(f"[red]未知主题或属性: {theme_name}[/red]")
    console.print(f"[dim]可用预设: {', '.join(THEME_PRESETS.keys())}[/dim]")
...
def _handle_directives_command(cmd: str, config: Config) -> None:
    ...
    if len(parts) < 2:
        if not config.user_directives:
            console.print("[dim]当前没有自定义指令[/dim]")
        else:
            console.print("[bold]自定义指令:[/bold]")
            for i, directive in enumerate(config.user_directives, 1):
                console.print(f"  {i}. {directive}")
        console.print(
            "\n[dim]用法:\n"
            "  /directives add <text>       — 添加指令\n"
            "  /directives rm <num>         — 删除指令\n"
    ...
    if sub.startswith("rm "):
        try:
            ...
            if 0 <= idx < len(config.user_directives):
                ...
                console.print(f"[red]无效编号: {idx + 1}[/red]")
        except ValueError:
            console.print("[red]请提供有效编号[/red]")
        return
    ...
    if sub == "clear":
        ...
        config.save_workspace()
        console.print(f"[green]✓ 已清空 {count} 条指令[/green]")
        return
    ...
    if sub == "proposals":
        ...
        if not proposals:
            console.print("[dim]暂无自动学习的提议[/dim]")
        else:
            console.print("[bold]行为模式提议:[/bold]")
            for p in proposals:
    ...
    if sub.startswith("accept "):
        ...
        if accept_directive(proposal_id, config):
            console.print(f"[green]✓ 已接受提议 {proposal_id} 为规则[/green]")
        else:
    ...
    if sub.startswith("reject "):
        ...
        if reject_directive(proposal_id):
            console.print(f"[yellow]✗ 已拒绝提议 {proposal_id}[/yellow]")
        else:
            console.print(f"[red]提议 {proposal_id} 不存在或已处理[/red]")
        return
...
def _handle_rule_command(cmd: str, config: Config) -> None:
    ...
    def _print_rules():
        ...
        if not entries:
            console.print("[dim]尚未配置 workspace 规则[/dim]")
            return
        console.print("[bold]Workspace 规则:[/bold]")
    ...
    if len(parts) < 2:
        ...
        console.print(
            ...
            "  /rule list                 — 列出规则\n"
            "  /rule add <text>           — 添加规则\n"
            "  /rule remove <num>         — 删除规则（别名: rm）\n"
            "  /rule edit                 — 用编辑器打开规则文件\n"
    ...
    if sub.startswith("add "):
        text = sub[4:].strip()
        if not text:
            console.print("[red]请提供规则内容[/red]")
            return
        entries = _load_entries()
        entries.append({"text": text, "enabled": True})
        _save_entries(entries)
        console.print(f"[green]✓ 已添加规则: {text}[/green]")
        return
    ...
    if sub == "edit":
        ...
        console.print("[green]✓ 规则文件已更新[/green]")
        return
    ...
    if sub.startswith("edit "):
        ...
        if len(parts2) < 2:
            console.print("[red]用法: /rule edit <编号> <新内容>[/red]")
            return
        try:
            idx = int(parts2[0]) - 1
        except ValueError:
        ...
        _save_entries(entries)
        console.print(f"[green]✓ 已更新规则 #{idx + 1}: {new_text}[/green]")
        return
    ...
    if sub.startswith("enable "):
        ...
        _save_entries(entries)
        console.print(f"[green]✓ 已启用规则 #{idx + 1}[/green]")
        return
    ...
    if sub.startswith("disable "):
        ...
        _save_entries(entries)
        console.print(f"[green]✓ 已禁用规则 #{idx + 1}[/green]")
        return
    ...
    if (
        ...
        if sub.startswith("remove "):
            ...
            num_part = ""
        if not num_part:
            console.print("[red]请提供编号，如 /rule remove 1 或 /rule rm 1[/red]")
            return
        try:
            idx = int(num_part) - 1
        except ValueError:
        ...
        _save_entries(entries)
        console.print(f"[green]✓ 已删除规则: {removed.get('text', '')}[/green]")
        return

    if sub == "clear":
        if rules_file.exists():
            rules_file.unlink()
            console.print("[green]✓ 规则已清空[/green]")
        else:
            console.print("[dim]没有规则可清空[/dim]")
        return
